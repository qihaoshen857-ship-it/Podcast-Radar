...
def resolve_app_dir() -> Path:
    ...
    if getattr(sys, "frozen", False):
        # Keep the legacy support-directory name so an app rename never splits
        # the user's downloads, settings, cache, or research library.
...
def browser_is_available(browser: str) -> bool:
    ...
    return any(shutil.which(command) for command in command_names.get(browser, []))
...
def main() -> None:
    ...
    if "--ui-smoke-check" in sys.argv:
        ...
        try:
            result = exercise_embodied_ui(app, sys.modules[__name__])
            result.update(version=APP_VERSION, frozen=bool(getattr(sys, "frozen", False)))
        except Exception as exc:
            result = {"ok": False, "error": f"{type(exc).__name__}: {exc}"}
        finally:
            root.destroy()
        output = Path(sys.argv[sys.argv.index("--ui-smoke-check") + 1])
