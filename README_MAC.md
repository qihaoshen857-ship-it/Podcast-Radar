...
1. 自动搜索 AI 产业、AI 创业、具身智能、天气农业和科学养生领域的重要播客更新，或读取 YouTube / 小宇宙节目列表并下载音频。
...
当前公开版本为 `v0.4.45`。人物监控访谈已经接入与今日雷达相同的转录整理流程；英文标题、阅读排版、天气 ENSO 权重和任务进度反馈也已同步更新。

## 首次安装

需要 macOS 和网络环境。第一次打开应用会弹出翻译服务配置，可填写自己的阿里云百炼或 DeepSeek API Key；跳过后仍可使用节目发现和纯本地 TXT 转录。极速云端 ASR 只使用阿里云 Key。
...
## 设置翻译服务、转录模式与 API Key

首次打开会直接显示配置窗口，也可稍后进入 `设置` 页修改：

1. 在阿里云百炼和 DeepSeek 中选择默认翻译服务，并填写对应的 API Key。
2. 阿里云支持中文标题、中文纪要、全文译文和极速云端 ASR；DeepSeek 支持前三项，音频继续使用本地 Whisper。
3. 选择 `本地优先` 或 `极速云端`；长播客希望明显提速时选 `极速云端`。
4. 保持默认切分参数和 4 路并发，或按需调整；程序安全上限为 8 路。
5. 分别保存翻译设置和转录设置。

API Key 会保存到本机 `.env` 文件中。未配置当前翻译服务的 Key 时，中文标题、纪要和全文译文不可用，节目发现与本地 Whisper 仍可使用。便携包导出脚本会排除 `.env`。

## 研究播客自动更新

应用打开后会自动读取 `双层雷达`，覆盖 AI 产业、AI 创业、具身智能、天气农业与科学养生播客 RSS，并补充精选 YouTube 频道源和小宇宙节目页，解析真实音频地址或视频入口和封面。新增的 `AI创业` 主题聚焦创始人故事、产品从 0 到 1、获客和商业化，会过滤纯模型新闻、估值争论和产业快讯。主界面可单独点击 `AI`、`AI创业`、`机器人`、`天气` 或 `养生` 分主题刷新。当前保留的手动链接模式仍可用于 YouTube、频道、播放列表和小宇宙页面。
...
v0.4.45 新增“机器人”按钮，聚焦人形机器人技术、零部件、创业者访谈和量产应用。详见 [栏目与评分说明](docs/EMBODIED_INTELLIGENCE.md)。
