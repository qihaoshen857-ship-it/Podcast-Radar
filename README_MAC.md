...
1. 自动搜索 AI、天气、农业领域重要播客更新，或读取 YouTube / 小宇宙节目列表并下载音频。
...
应用打开后会自动读取 `双层雷达`，覆盖 AI、天气、ENSO、气候与农业市场相关播客 RSS，并补充精选 AI / 养生 YouTube 官方频道源和小宇宙节目页，解析真实音频地址或视频入口和封面，并分成 `AI 播客` / `天气农业` / `养生` 区域展示。主界面也可以单独点击 `AI`、`天气农业` 或 `养生` 分主题刷新。当前保留的手动链接模式仍可用于 YouTube、频道、播放列表和小宇宙页面。
