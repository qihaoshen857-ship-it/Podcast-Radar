...
1. 自动搜索 AI 产业、AI 创业、具身智能、天气农业和科学养生领域的重要播客更新，或从 YouTube / 小宇宙读取节目列表并下载音频。
...
首次打开应用会直接显示翻译服务配置：可选择阿里云百炼或 DeepSeek，并填写自己的 API Key。
阿里云可用于中文标题、研究纪要、全文译文和极速云端 ASR；DeepSeek 用于前三项，音频转录仍走本地 Whisper。
暂不配置也可以继续发现节目和本地转录，使用翻译功能时应用会再次提示。两个 Key 都只保存在本机 `.env`。
...
- 打开应用后会自动读取 `双层雷达`：AI 产业、AI 创业、具身智能、天气农业和科学养生播客 RSS，精选 YouTube 官方频道源与小宇宙节目页，并分区展示。
- 首次打开先选择阿里云百炼或 DeepSeek 作为翻译服务并填写自己的 API Key；也可点击 `稍后配置`，继续使用节目发现和本地 Whisper。
...
- 选中卡片后点击 `转录整理选中`：先下载到隐藏缓存，再按设置的 `本地优先` / `极速云端` 模式生成转录，并通过当前选定的翻译服务生成中文 Markdown 研究笔记。
